...
- x86/x64 的 Debug/Release 全矩阵 Rebuild 通过且无编译或链接警告；每个配置下 217 项断言全部通过，其中包含 File.Enumerate/File.Query 和既有管理操作端到端验证。

## 下一步

1. 定稿 File ChannelData 的打开、窗口、结束和错误语义，并实现只读下载；
