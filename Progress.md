...
- x86/x64 的 Debug/Release 全矩阵 Rebuild 通过且无编译或链接警告；每个配置下 211 项断言全部通过，其中包含授权门禁默认策略、回调结果传播、真实认证 ClientId 以及既有管理操作端到端验证。

## 下一步

1. 在统一权限门禁上接入 Process.Terminate 等首批 Control 操作；
