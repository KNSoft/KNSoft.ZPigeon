    ...
    _Inout_ PZP_AUDIO_DEVICE Record)
{
    ...
    {
        Record->Volume = (ULONG)(min(max(Scalar, 0.0f), 1.0f) * ZP_AUDIO_VOLUME_MAX + 0.5f);
    ...
    _Out_ PZP_AUDIO_DEVICE_BUILDER Builder)
{
    ...
    for (Index = 0; SUCCEEDED(Result) && Index < Count; Index++)
    {
        ...
        if (SUCCEEDED(Result)) Result = IMMDevice_GetState(Device, &Record->State);
    ...
    _Out_ PULONG ResponseLength)
{
    ...
    *Response = NT_SUCCESS(Status) ? Mem_Alloc(*ResponseLength) : NULL;
    if (!NT_SUCCESS(Status) || *Response == NULL) return NT_SUCCESS(Status) ? STATUS_NO_MEMORY : Status;
    ...
    _Outptr_result_maybenull_ PZP_CLIENT_AUDIO_CHANNEL* Channel)
{
    ...
    *Channel = NULL;
    switch (OperationId)
    {
        ...
        case ZP_AUDIO_OPERATION_CONTROL_SESSION:
        ...
        case ZP_AUDIO_OPERATION_OPEN_STREAM:
            ...
            *Response = Mem_Alloc(*ResponseLength);
            ...
            if (!NT_SUCCESS(Status))
            {
                Mem_Free(*Response);
                *Response = NULL;
                ZpAudio_CommitChannel(AudioChannel, FALSE);
