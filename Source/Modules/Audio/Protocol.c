    ...
    _Out_opt_ PZP_AUDIO_DEVICE_VIEW Device)
{
    ...
    Status = ZpCodec_ReadByte(Reader, &Local.Flow);
    ...
    if (NT_SUCCESS(Status)) Status = ZpCodec_ReadUInt16(Reader, &Local.Volume);
    ...
    _Out_ PULONG BytesWritten)
{
    ...
    for (Index = 0; NT_SUCCESS(Status) && Index < Count; Index++)
    {
        ...
        Status = ZpCodec_WriteByte(&Writer, Device->Flow);
        if (NT_SUCCESS(Status)) Status = ZpCodec_WriteByte(&Writer, Device->State);
        if (NT_SUCCESS(Status)) Status = ZpCodec_WriteByte(&Writer, Device->Flags);
        if (NT_SUCCESS(Status)) Status = ZpCodec_WriteUInt16(&Writer, Device->Volume);
    ...
    _Out_ PULONG BytesWritten)
{
    ...
    for (Index = 0; NT_SUCCESS(Status) && Index < Count; Index++)
    {
        ...
        if (NT_SUCCESS(Status)) Status = ZpCodec_WriteByte(&Writer, Session->State);
        if (NT_SUCCESS(Status)) Status = ZpCodec_WriteByte(&Writer, Session->Flags);
        if (NT_SUCCESS(Status)) Status = ZpCodec_WriteUInt16(&Writer, Session->Volume);
