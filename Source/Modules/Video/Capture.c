    ...
    _Out_ PZP_VIDEO_IMAGE Image)
{
    ...
    PVOID Data;
    PBYTE Output;
    ULONG Width = Capture->Width, Height = Capture->Height, Stride;
    HRESULT Result;
    ...
    if (SUCCEEDED(Result) && (Width != Capture->Width || Height != Capture->Height))
    {
        Result = IWICImagingFactory_CreateBitmapScaler(Capture->ImagingFactory, &Scaler);
        if (SUCCEEDED(Result))
        {
