...
typedef BYTE ZP_PORTABLE_MANAGER_STRING;

#define ZpPortableManagerFriendlyName ((ZP_PORTABLE_MANAGER_STRING)0)
#define ZpPortableManagerManufacturer ((ZP_PORTABLE_MANAGER_STRING)1)
#define ZpPortableManagerDescription ((ZP_PORTABLE_MANAGER_STRING)2)
...
typedef BYTE ZP_CLIENT_PORTABLE_CHANNEL_TYPE;

#define ZpClientPortableChannelRead ((ZP_CLIENT_PORTABLE_CHANNEL_TYPE)0)
#define ZpClientPortableChannelWrite ((ZP_CLIENT_PORTABLE_CHANNEL_TYPE)1)
    ...
    _In_opt_ PVOID Context)
{
    ...
    for (;;)
    {
        ...
        Status = ZpPortable_SendLocked(Object,
    ...
    _Outptr_result_maybenull_ PZP_CLIENT_LOCAL_CHANNEL* Channel)
{
    ...
    if (OperationId == ZP_PORTABLE_DEVICE_OPERATION_ENUMERATE_DEVICES)
    {
        if (PayloadLength != 0) OperationStatus = ZpStatus_FromNtStatus(STATUS_INVALID_PARAMETER);
        else
        {
            Result = ZpPortable_EnumerateDevices(Response, ResponseLength);
            OperationStatus = ZpPortable_StatusFromHResult(Result);
        }
    }
    else if (OperationId == ZP_PORTABLE_DEVICE_OPERATION_ENUMERATE_OBJECTS)
    {
        Status = ZpPortable_DecodeObjectPageRequest(Payload, PayloadLength, &PageRequest);
        if (!NT_SUCCESS(Status)) OperationStatus = ZpStatus_FromNtStatus(Status);
        else
        {
            Result = ZpPortable_EnumerateObjects(&PageRequest, Response, ResponseLength);
            OperationStatus = ZpPortable_StatusFromHResult(Result);
        }
    }
    else if (OperationId == ZP_PORTABLE_DEVICE_OPERATION_CREATE_FOLDER)
    {
        Status = ZpPortable_DecodeNameRequest(Payload, PayloadLength, &NameRequest);
        if (!NT_SUCCESS(Status)) OperationStatus = ZpStatus_FromNtStatus(Status);
        else
        {
            Result = ZpPortable_CreateFolder(&NameRequest);
            OperationStatus = ZpPortable_StatusFromHResult(Result);
        }
    }
    else if (OperationId == ZP_PORTABLE_DEVICE_OPERATION_DELETE)
    {
        Status = ZpPortable_DecodeObjectRequest(Payload, PayloadLength, &ObjectRequest);
        if (!NT_SUCCESS(Status)) OperationStatus = ZpStatus_FromNtStatus(Status);
        else
        {
            Result = ZpPortable_Delete(&ObjectRequest);
            OperationStatus = ZpPortable_StatusFromHResult(Result);
        }
    }
    else if (OperationId == ZP_PORTABLE_DEVICE_OPERATION_RENAME)
    {
        Status = ZpPortable_DecodeNameRequest(Payload, PayloadLength, &NameRequest);
        if (!NT_SUCCESS(Status)) OperationStatus = ZpStatus_FromNtStatus(Status);
        else
        {
            Result = ZpPortable_Rename(&NameRequest);
            OperationStatus = ZpPortable_StatusFromHResult(Result);
        }
    }
    else if (OperationId == ZP_PORTABLE_DEVICE_OPERATION_OPEN_READ)
    {
        Status = ZpPortable_DecodeObjectRequest(Payload, PayloadLength, &ObjectRequest);
        OperationStatus = NT_SUCCESS(Status) ?
                              ZpPortable_CreateReadChannel(Client, &ObjectRequest, &PortableChannel, &FileSize) :
    ...
    else if (OperationId == ZP_PORTABLE_DEVICE_OPERATION_OPEN_WRITE)
    {
    ...
    if (ZpStatus_IsSuccess(OperationStatus) && PortableChannel != NULL) *Channel = &PortableChannel->Header;
    else if (PortableChannel != NULL)
    {
        ZpPortable_CommitChannel(&PortableChannel->Header, FALSE);
    }
    if (SUCCEEDED(InitializeResult)) CoUninitialize();
