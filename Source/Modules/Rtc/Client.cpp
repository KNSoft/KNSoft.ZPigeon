...
static const WCHAR ZpRtcPage[] =
    ...
    L"pc.addEventListener('icegatheringstatechange',()=>{if(pc.iceGatheringState==='complete'){clearTimeout(timeout);resolve()}},{once:false})});"
    ...
    L"if(!value.startsWith('OFFER\\n'))return;pc=new RTCPeerConnection({iceServers:servers.map(url=>({urls:url}))});"
    L"pc.ondatachannel=event=>{channel=event.channel;channel.binaryType='arraybuffer';channel.bufferedAmountLowThreshold=1048576;"
    L"channel.onopen=()=>post('OPEN');channel.onclose=()=>post('CLOSED');"
    L"channel.onmessage=event=>{if(event.data==='PING')channel.send('PONG')}};"
    L"pc.onconnectionstatechange=()=>{if(pc.connectionState==='failed'||pc.connectionState==='closed')post('CLOSED')};"
    L"await pc.setRemoteDescription({type:'offer',sdp:value.slice(6)});await pc.setLocalDescription(await pc.createAnswer());"
    ...
    _Out_ PULONG ResponseLength)
{
    ...
    if (ZpStatus_IsSuccess(Session->OpenStatus))
    {
        Status = ZpRtc_EncodeAnswer(Session->Answer,
                                    ...
                                    0,
                                    ResponseLength);
        *Response = NT_SUCCESS(Status) ? (PBYTE)Mem_Alloc(*ResponseLength) : NULL;
