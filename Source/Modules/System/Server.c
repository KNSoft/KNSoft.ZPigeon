...
typedef struct _ZP_SYSTEM_STATUS_CONTEXT
{
    ZP_REQUEST_STATUS_CALLBACK Callback;
    PVOID Context;
} ZP_SYSTEM_STATUS_CONTEXT, *PZP_SYSTEM_STATUS_CONTEXT;

static
VOID
NTAPI
ZpSystem_StatusComplete(
    _In_ ZP_REQUEST_HANDLE Request,
    _In_ ZP_STATUS Status,
    _In_ PCZP_BUFFER_VIEW Payload,
    _In_opt_ PVOID Context)
{
    PZP_SYSTEM_STATUS_CONTEXT SystemContext = Context;

    if (ZpStatus_IsSuccess(Status) && Payload->Length != 0)
    {
        Status = ZpStatus_FromNtStatus(STATUS_DATA_ERROR);
    }
    SystemContext->Callback(Request, Status, SystemContext->Context);
...
ZpServer_ProbeConnection(
    ...
    _Out_ ZP_REQUEST_HANDLE* Request)
{
    PZP_SYSTEM_STATUS_CONTEXT SystemContext;
    NTSTATUS Status;

    if (Callback == NULL) return STATUS_INVALID_PARAMETER;
    SystemContext = Mem_Alloc(sizeof(*SystemContext));
    if (SystemContext == NULL) return STATUS_NO_MEMORY;
    SystemContext->Callback = Callback;
    SystemContext->Context = Context;
    Status = ZpServer_SendRequest(Connection,
                                  ZP_SYSTEM_MODULE_ID,
                                  ZP_SYSTEM_OPERATION_PROBE,
                                  TimeoutMilliseconds,
                                  NULL,
                                  0,
                                  ZpSystem_StatusComplete,
                                  SystemContext,
                                  Request);
    if (!NT_SUCCESS(Status)) Mem_Free(SystemContext);
    return Status;
}
