    ...
    _In_ PVOID Context)
{
    ...
    for (;;)
    {
        ...
        Status = ZpMessage_EncodeChannelData(Channel->Header.ChannelId,
                                             ...
                                             Body,
                                             sizeof(ULONG) + ZP_SERIAL_CHUNK_SIZE,
                                             &BodyLength);
        if (NT_SUCCESS(Status) && Channel->Header.Pending)
        {
...
ZpSerial_Execute(
    ...
    _Out_ PULONG ResponseLength,
    _Outptr_result_maybenull_ PZP_CLIENT_SERIAL_CHANNEL* Channel)
{
    ...
    *Response = Mem_Alloc(*ResponseLength);
    ...
    if (!NT_SUCCESS(Status))
    {
        Mem_Free(*Response);
        *Response = NULL;
        ZpSerial_CommitChannel(OpenedChannel, FALSE);
