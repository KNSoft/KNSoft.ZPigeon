...
{
    PZP_PROCESS_CONTEXT ProcessContext = Context;
    ZP_PROCESS_MODULE_LIST_VIEW Modules;

    if (ZpStatus_IsSuccess(Status))
    {
        Status = ZpStatus_FromNtStatus(
            ZpProcess_DecodeModuleList(Payload->Buffer,
                                       Payload->Length,
                                       &Modules));
    }
...
VOID
NTAPI
ZpProcess_MemoryAllocationsComplete(
    _In_ ZP_REQUEST_HANDLE Request,
    _In_ ZP_STATUS Status,
    _In_ PCZP_BUFFER_VIEW Payload,
    _In_opt_ PVOID Context)
{
    PZP_PROCESS_CONTEXT ProcessContext = Context;
    ZP_PROCESS_MEMORY_ALLOCATION_MAP_VIEW Map;

    if (ZpStatus_IsSuccess(Status))
    {
        Status = ZpStatus_FromNtStatus(
            ZpProcess_DecodeMemoryAllocations(Payload->Buffer, Payload->Length, &Map));
    }
    ProcessContext->Callback.MemoryAllocations(Request,
                                               Status,
...
NTSTATUS
NTAPI
ZpServer_EnumerateProcessModules(
    _In_ ZP_CONNECTION_HANDLE Connection,
    _In_ ULONG ProcessId,
    _In_ ULONGLONG CreateTime,
    _In_ ULONG TimeoutMilliseconds,
    _In_ ZP_PROCESS_MODULES_CALLBACK Callback,
    _In_opt_ PVOID Context,
    _Out_ ZP_REQUEST_HANDLE* Request)
{
    ZP_PROCESS_CALLBACK ProcessCallback;
    ...
    ProcessCallback.Modules = Callback;
    return ZpProcess_Send(Connection,
                          ZP_PROCESS_OPERATION_ENUMERATE_MODULES,
                          TimeoutMilliseconds,
                          Payload,
                          PayloadLength,
                          ZpProcess_ModulesComplete,
...
ZpServer_QueryProcessMemoryMap(
    ...
    _In_ ULONGLONG CreateTime,
    _In_ ULONG TimeoutMilliseconds,
    _In_ ZP_PROCESS_MEMORY_ALLOCATIONS_CALLBACK Callback,
    _In_opt_ PVOID Context,
    _Out_ ZP_REQUEST_HANDLE* Request)
{
    ZP_PROCESS_CALLBACK ProcessCallback;
    ...
    if (Callback == NULL) return STATUS_INVALID_PARAMETER;
    ProcessCallback.MemoryAllocations = Callback;
    Status = ZpProcess_EncodeQuery(ProcessId,
                                   CreateTime,
                                   Payload,
                                   sizeof(Payload),
    ...
    return NT_SUCCESS(Status) ?
               ZpProcess_Send(Connection,
                               ZP_PROCESS_OPERATION_OPEN_MEMORY_MAP,
                              TimeoutMilliseconds,
                              Payload,
                              PayloadLength,
                               ZpProcess_MemoryAllocationsComplete,
...
NTSTATUS
NTAPI
ZpServer_QueryProcessMemoryRegions(
    _In_ ZP_CONNECTION_HANDLE Connection,
    ...
    _Out_ ZP_REQUEST_HANDLE* Request)
{
    ...
    if (Callback == NULL) return STATUS_INVALID_PARAMETER;
    Status = ZpProcess_EncodeMemoryMapQuery(SnapshotId,
                                           AllocationIndex,
                                           Payload,
                                           sizeof(Payload),
                                           &PayloadLength);
    ...
    return ZpProcess_Send(Connection,
                          ZP_PROCESS_OPERATION_QUERY_MEMORY_REGIONS,
                          TimeoutMilliseconds,
                          Payload,
                          PayloadLength,
                          ZpProcess_MemoryMapComplete,
...
NTSTATUS
NTAPI
ZpServer_CloseProcessMemoryMap(
    _In_ ZP_CONNECTION_HANDLE Connection,
    _In_ ULONG SnapshotId,
    _In_ ULONG TimeoutMilliseconds,
    _In_ ZP_REQUEST_STATUS_CALLBACK Callback,
    ...
    _Out_ ZP_REQUEST_HANDLE* Request)
{
    ...
    if (Callback == NULL) return STATUS_INVALID_PARAMETER;
    Status = ZpProcess_EncodeMemoryMapClose(SnapshotId,
                                           Payload,
                                           sizeof(Payload),
                                           &PayloadLength);
    if (!NT_SUCCESS(Status)) return Status;
    ProcessCallback.Status = Callback;
    return ZpProcess_Send(Connection,
                          ZP_PROCESS_OPERATION_CLOSE_MEMORY_MAP,
                          TimeoutMilliseconds,
                          Payload,
                          PayloadLength,
                          ZpProcess_StatusComplete,
