    ...
    _Out_ PULONG ResponseLength)
{
    ...
    for (Index = 0; Index < TargetCount; Index++)
    {
        ...
        if (CandidateLength > ZP_RESPONSE_MAX_PAYLOAD_SIZE)
        {
    ...
    if (NT_SUCCESS(CodecStatus))
    {
        *Response = Mem_Alloc(*ResponseLength);
        if (*Response == NULL) CodecStatus = STATUS_NO_MEMORY;
    }
    if (NT_SUCCESS(CodecStatus))
    {
        CodecStatus = ZpEventLog_EncodePage(HasMore,
...
#define ZP_EVENT_LOG_GET_CONFIG(Property, Type) \
    ...
    Info.Enabled = Value.BooleanVal != FALSE;
    ZP_EVENT_LOG_GET_CONFIG(EvtChannelConfigType, EvtVarTypeUInt32);
    Info.Type = (BYTE)Value.UInt32Val;
    ...
    _Out_ PULONG ResponseLength)
{
    ...
    for (;;)
    {
        ...
        Status = ZpEventLog_EncodeChannel(Buffer, Required - 1, NULL, 0, &ChannelLength);
        if (!NT_SUCCESS(Status)) break;
        if (ChannelLength > ZP_RESPONSE_MAX_PAYLOAD_SIZE - OutputLength)
        {
            Status = STATUS_BUFFER_OVERFLOW;
            break;
        }
        if (OutputLength + ChannelLength > Capacity)
        {
            do Capacity = min(Capacity * 2, ZP_RESPONSE_MAX_PAYLOAD_SIZE);
            while (Capacity < OutputLength + ChannelLength);
            Values = Mem_ReAlloc(Output, Capacity);
            if (Values == NULL)
            {
                Status = STATUS_NO_MEMORY;
                break;
        ...
        Status = ZpEventLog_EncodeChannel(Buffer,
                                          Required - 1,
                                          Output + OutputLength,
                                          Capacity - OutputLength,
                                          &ChannelLength);
        if (!NT_SUCCESS(Status)) break;
        OutputLength += ChannelLength;
        Count++;
        Mem_Free(Buffer);
        Buffer = NULL;
    }
    ...
    {
        Status = ZpEventLog_EncodeChannelListHeader(Count, Output);
    ...
    if (Error != ERROR_SUCCESS || !NT_SUCCESS(Status)) Mem_Free(Output);
