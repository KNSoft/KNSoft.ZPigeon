...
ZpService_QueryDescription(
    _In_ SC_HANDLE Service)
{
    LPSERVICE_DESCRIPTIONW Information = NULL;
    PUNICODE_STRING Description = NULL, ResolvedDescription;
    PWCHAR Resolved = NULL;
    ULONG DescriptionLength, ResolvedLength;

    if (!ZpStatus_IsSuccess(
            ZpService_QueryConfig2(Service, SERVICE_CONFIG_DESCRIPTION, (PVOID*)&Information)) ||
        Information->lpDescription == NULL)
    {
        goto Cleanup;
    }
    DescriptionLength = (ULONG)wcslen(Information->lpDescription);
    ...
    RtlCopyMemory(Description->Buffer, Information->lpDescription, Description->Length);
    ...
    _Out_ PULONG PayloadLength)
{
    ...
    for (Index = 0; Index < Count; Index++)
    {
        Services[Index].ServiceType = (USHORT)Entries[Index].ServiceStatusProcess.dwServiceType;
        Services[Index].CurrentState = (BYTE)Entries[Index].ServiceStatusProcess.dwCurrentState;
        Services[Index].ControlsAccepted = (USHORT)Entries[Index].ServiceStatusProcess.dwControlsAccepted;
        Services[Index].ProcessId = Entries[Index].ServiceStatusProcess.dwProcessId;
        Services[Index].StartType = ZP_SERVICE_START_TYPE_UNKNOWN;
        ...
        if (Service != NULL)
        {
            Allocations[Index].Description = ZpService_QueryDescription(Service);
            ...
            {
                Services[Index].StartType = (BYTE)Allocations[Index].Config->dwStartType;
    ...
    _Out_ PULONG PayloadLength)
{
    ...
    Description = ZpService_QueryDescription(Service);
    ...
    if (Info.RecoverySupported)
    {
        ...
        Info.FailureActionsOnNonCrashFailures =
            FailureFlag.fFailureActionsOnNonCrashFailures != FALSE;
        ...
        for (Index = 0; Index < 4; Index++)
        {
            ...
            {
                case 0:
                    Info.FirstFailureAction = (BYTE)LastAction;
                    break;

                case 1:
                    Info.SecondFailureAction = (BYTE)LastAction;
                    break;

                case 2:
                    Info.ThirdFailureAction = (BYTE)LastAction;
                    break;

                default:
                    Info.SubsequentFailureAction = (BYTE)LastAction;
                    break;
            }
        }
    ...
    Info.ServiceType = (USHORT)ServiceStatus.dwServiceType;
    Info.CurrentState = (BYTE)ServiceStatus.dwCurrentState;
    Info.ControlsAccepted = (USHORT)ServiceStatus.dwControlsAccepted;
    Info.ProcessId = ServiceStatus.dwProcessId;
    Info.StartType = (BYTE)Config->dwStartType;
    Info.ErrorControl = (BYTE)Config->dwErrorControl;
    Info.DelayedAutoStart = DelayedInfo.fDelayedAutostart != FALSE;
    ...
    _Out_ PULONG ResponseLength)
{
    ...
    else if (OperationId == ZP_SERVICE_OPERATION_QUERY)
    {
        Status = ZpService_DecodeQuery(Request, RequestLength, &Name);
        return NT_SUCCESS(Status) ? ZpService_Query(&Name, Response, ResponseLength) :
                                    ZpStatus_FromNtStatus(Status);
    }
    if (OperationId == ZP_SERVICE_OPERATION_CONTROL)
    {
        Status = ZpService_DecodeControl(Request, RequestLength, &Control, &Name, &Argument);
        Result = NT_SUCCESS(Status) ? ZpService_Control(Control, &Name, &Argument) : ZpStatus_FromNtStatus(Status);
    }
    else if (OperationId == ZP_SERVICE_OPERATION_CONFIGURE_GENERAL)
    {
        Status = ZpService_DecodeConfig(Request, RequestLength, &Config);
        Result = NT_SUCCESS(Status) ? ZpService_Configure(&Config) : ZpStatus_FromNtStatus(Status);
    }
    else if (OperationId == ZP_SERVICE_OPERATION_CONFIGURE_RECOVERY)
    {
        Status = ZpService_DecodeRecoveryConfig(Request, RequestLength, &RecoveryConfig);
        Result = NT_SUCCESS(Status) ? ZpService_ConfigureRecovery(&RecoveryConfig) : ZpStatus_FromNtStatus(Status);
    }
    else if (OperationId == ZP_SERVICE_OPERATION_CONFIGURE_ACCOUNT)
    {
        Status = ZpService_DecodeAccountConfig(Request, RequestLength, &AccountConfig);
        Result = NT_SUCCESS(Status) ? ZpService_ConfigureAccount(&AccountConfig) : ZpStatus_FromNtStatus(Status);
    }
    else
    {
        Result = ZpStatus_FromNtStatus(STATUS_NOT_SUPPORTED);
    }
    return Result;
}
