    ...
    _Out_opt_ PZP_SERVICE_RECORD_VIEW Record)
{
    ...
    Status = ZpCodec_ReadUInt16(Reader, &LocalRecord.ServiceType);
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(Reader, &LocalRecord.CurrentState);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadUInt16(Reader, &LocalRecord.ControlsAccepted);
    }
    if (NT_SUCCESS(Status))
    {
    ...
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(Reader, &LocalRecord.StartType);
    }
    if (NT_SUCCESS(Status))
    {
    ...
    _Out_ PULONG BytesWritten)
{
    ...
    for (Index = 0; Index < ServiceCount; Index++)
    {
        ...
        RequiredSize += 2 * sizeof(USHORT) + 2 * sizeof(BYTE) + 5 * sizeof(ULONG) +
    ...
    *BytesWritten = (ULONG)RequiredSize;
    ...
    for (Index = 0; Index < ServiceCount; Index++)
    {
        ZpWire_WriteUInt16(&Cursor, Services[Index].ServiceType);
        *Cursor++ = Services[Index].CurrentState;
        ZpWire_WriteUInt16(&Cursor, Services[Index].ControlsAccepted);
        ZpWire_WriteUInt32(&Cursor, Services[Index].ProcessId);
        *Cursor++ = Services[Index].StartType;
    ...
    _Out_ PULONG BytesWritten)
{
    ...
    *BytesWritten = (ULONG)RequiredSize;
    ...
    ZpCodec_InitializeWriter(&Writer, Buffer, BufferSize);
    Status = ZpCodec_WriteUInt16(&Writer, Info->ServiceType);
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, Info->CurrentState);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteUInt16(&Writer, Info->ControlsAccepted);
    }
    if (NT_SUCCESS(Status))
    {
    ...
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, Info->StartType);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, Info->ErrorControl);
    }
    if (NT_SUCCESS(Status))
    {
    ...
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, Info->FirstFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, Info->SecondFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, Info->ThirdFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, Info->SubsequentFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
    ...
    _Out_ PZP_SERVICE_INFO_VIEW View)
{
    ...
    ZpCodec_InitializeReader(&Reader, Payload, PayloadLength);
    Status = ZpCodec_ReadUInt16(&Reader, &View->ServiceType);
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &View->CurrentState);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadUInt16(&Reader, &View->ControlsAccepted);
    }
    if (NT_SUCCESS(Status))
    {
    ...
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &View->StartType);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &View->ErrorControl);
    }
    if (NT_SUCCESS(Status))
    {
    ...
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &View->FirstFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &View->SecondFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &View->ThirdFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &View->SubsequentFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
    ...
    _Out_ PULONG BytesWritten)
{
    ...
    *BytesWritten = (ULONG)RequiredSize;
    ...
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, (BYTE)Config->FirstFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, (BYTE)Config->SecondFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, (BYTE)Config->ThirdFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteByte(&Writer, (BYTE)Config->SubsequentFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_WriteString(&Writer, Config->ServiceName, Config->ServiceNameLength);
    ...
    _Out_ PZP_SERVICE_RECOVERY_CONFIG_VIEW Config)
{
    ...
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &Config->FirstFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &Config->SecondFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &Config->ThirdFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadByte(&Reader, &Config->SubsequentFailureAction);
    }
    if (NT_SUCCESS(Status))
    {
        Status = ZpCodec_ReadString(&Reader, &Config->ServiceName);
