    ...
    _Out_ PULONG ResponseLength)
{
    ...
    if (Capabilities.UpsPresent)
    {
        ...
        _ultow_s(Battery.EstimatedTime, Detail, ARRAYSIZE(Detail), 10);
        Status = ZpAdministration_AddRecord(
            &Builder,
            ZpAdministrationKindUps,
            TRUE,
            (Battery.AcOnLine ? 1 : 0) | (Battery.Charging ? 2 : 0) | (Battery.Discharging ? 4 : 0),
            ((ULONGLONG)Battery.MaxCapacity << 32) | Battery.RemainingCapacity,
            L"UPS",
            L"不间断电源 (UPS)",
            Capabilities.BatteriesAreShortTerm ? L"短时备用电源" : NULL,
            Detail);
    }
