...
#define ZP_RDP_CHILD_SESSION_RUN_ONCE_KEY L"Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce"
#define ZP_RDP_CHILD_SESSION_RUN_ONCE_VALUE L"KNSoft.ZPigeon.ChildSession"
...
ZpRdpChildSession_ValidateParent(VOID)
{
    ...
    if (Bytes != sizeof(*ConnectState))
    {
        Status = ZpStatus_FromCode(ZpStatusWin32, ERROR_INVALID_DATA);
    }
    else if (*ConnectState != WTSActive)
    {
        Status = ZpStatus_FromCode(ZpStatusWin32, ERROR_CTX_WINSTATION_NOT_FOUND);
    }
    ...
    {
        Status = ZpStatus_FromCode(ZpStatusWin32, ERROR_NOT_LOGGED_ON);
    }
...
ZpRdpChildSession_Logoff(VOID)
{
    ...
    if (WTSGetChildSessionId(&SessionId))
    {
...
static
NTSTATUS
ZpRdpChildSession_OpenRunOnceKey(
    _Out_ PHANDLE Key)
{
    ...
    Status = RtlOpenCurrentUser(KEY_CREATE_SUB_KEY, &CurrentUser);
    if (!NT_SUCCESS(Status)) return Status;
    RtlInitUnicodeString(&KeyName, ZP_RDP_CHILD_SESSION_RUN_ONCE_KEY);
...
static
HRESULT
ZpRdpChildSession_RegisterWorker(VOID)
{
    ...
    ModuleLength = GetModuleFileNameW(NULL, ModulePath, ARRAYSIZE(ModulePath));
    if (ModuleLength == 0) return HRESULT_FROM_WIN32(GetLastError());
    if (ModuleLength == ARRAYSIZE(ModulePath))
    {
        return HRESULT_FROM_WIN32(ERROR_FILENAME_EXCED_RANGE);
    }
    CommandLength = _snwprintf_s(Command,
                                 ARRAYSIZE(Command),
                                 _TRUNCATE,
                                 L"\"%s\" %s",
                                 ModulePath,
                                 ZP_RDP_CHILD_SESSION_WORKER_ARGUMENT);
    if (CommandLength < 0)
    {
        return HRESULT_FROM_WIN32(ERROR_FILENAME_EXCED_RANGE);
    }
    Status = ZpRdpChildSession_OpenRunOnceKey(&Key);
    if (!NT_SUCCESS(Status)) return HRESULT_FROM_NT(Status);
    RtlInitUnicodeString(&ValueName, ZP_RDP_CHILD_SESSION_RUN_ONCE_VALUE);
    Status = NtSetValueKey(Key,
                           &ValueName,
                           0,
                           REG_SZ,
                           Command,
                           (CommandLength + 1) * sizeof(WCHAR));
    NtClose(Key);
...
static
VOID
ZpRdpChildSession_UnregisterWorker(VOID)
{
    UNICODE_STRING ValueName;
    HANDLE Key;

    if (!NT_SUCCESS(ZpRdpChildSession_OpenRunOnceKey(&Key))) return;
    RtlInitUnicodeString(&ValueName, ZP_RDP_CHILD_SESSION_RUN_ONCE_VALUE);
    NtDeleteValueKey(Key, &ValueName);
    NtClose(Key);
}
    ...
    _In_ PVOID Parameter)
{
    ...
    MSG Message;
    HRESULT Result;
    BOOLEAN Initialized = FALSE, ConnectionStarted = FALSE, WorkerRegistered = FALSE;
    ...
    if (SUCCEEDED(Result))
    {
        Result = ZpRdpChildSession_RegisterWorker();
        WorkerRegistered = SUCCEEDED(Result);
    }
    if (SUCCEEDED(Result))
    {
    ...
    if (WorkerRegistered) ZpRdpChildSession_UnregisterWorker();
