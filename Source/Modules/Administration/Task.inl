...
static ZP_STATUS ZpAdministration_EnumerateTasksImpl(
    _Outptr_result_bytebuffer_(*ResponseLength) PBYTE* Response,
    _Out_ PULONG ResponseLength);
static ZP_STATUS ZpAdministration_ControlTaskImpl(
    ...
    _In_ PCZP_ADMINISTRATION_CONTROL_VIEW Control)
{
    ...
    {
        Xml = ZpAdministration_CopyView(&Control->Argument);
        if (Xml == NULL)
        {
            Mem_Free(Identity);
            ...
            if (SUCCEEDED(Result))
            {
                ...
                switch (Control->Action)
                {
                    ...
                    case ZpAdministrationActionConfigure:
                        Path = SysAllocString(Identity);
                        XmlText = SysAllocString(Xml);
                        if (Path == NULL || XmlText == NULL)
                        {
                            Result = E_OUTOFMEMORY;
                        }
                        else
                        {
                            // Editing XML must preserve the task ACL and must not fire registration triggers.
                            Result = ITaskFolder_RegisterTask(Root,
                                                              Path,
                                                              XmlText,
                                                              TASK_UPDATE |
                                                              ...
                                                              Empty,
                                                              &Registered);
                            if (SUCCEEDED(Result)) IRegisteredTask_Release(Registered);
                        }
