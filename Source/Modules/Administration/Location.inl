    ...
    _In_ DOUBLE Value)
{
    ...
    return ZpCodec_WriteUInt64(Writer, Bits);
}
    ...
    _Out_ PULONG ResponseLength)
{
    ...
    if (SUCCEEDED(Result))
    {
        ...
        Status = ZpLocation_WriteDouble(&Writer, Latitude);
        if (NT_SUCCESS(Status)) Status = ZpLocation_WriteDouble(&Writer, Longitude);
        if (NT_SUCCESS(Status)) Status = ZpLocation_WriteDouble(&Writer, Radius);
        if (NT_SUCCESS(Status)) Status = ZpLocation_WriteDouble(&Writer, Altitude);
        if (NT_SUCCESS(Status)) Status = ZpLocation_WriteDouble(&Writer, AltitudeError);
