...
{
    return NameLength != 0 && NameLength <= ZP_WMI_MAX_CELL_LENGTH &&
           ValueLength <= ZP_WMI_MAX_CELL_LENGTH && Name != NULL &&
    ...
    _Out_ PULONG BytesWritten)
{
    ...
    if (!ZpWmi_IsCellValid(Name, NameLength, Value, ValueLength)) return STATUS_INVALID_PARAMETER;
    RequiredSize = sizeof(USHORT) + 2 * sizeof(ULONG) +
                   ((ULONGLONG)NameLength + ValueLength) * sizeof(WCHAR);
    ...
    *BytesWritten = (ULONG)RequiredSize;
    ...
    Cursor = Buffer;
    ZpWire_WriteUInt16(&Cursor, Type);
    ZpWire_WriteString(&Cursor, Name, NameLength);
    ZpWire_WriteString(&Cursor, Value, ValueLength);
    ...
    _Out_opt_ PZP_WMI_CELL Cell)
{
    ...
    Status = ZpCodec_ReadUInt16(Reader, &Local.Type);
    ...
    _Out_ PULONG BytesWritten)
{
    ...
    for (RowIndex = 0; RowIndex < RowCount; RowIndex++)
    {
        ...
        for (CellIndex = 0; CellIndex < Rows[RowIndex].CellCount; CellIndex++)
        {
            ...
            Status = ZpWmi_EncodeCell(Cell->Type,
                                      Cell->Name,
                                      Cell->NameLength,
                                      Cell->Value,
                                      Cell->ValueLength,
