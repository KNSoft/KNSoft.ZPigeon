    ...
    _Inout_ PZP_RECORDING_JOB Job)
{
    ...
    while (SUCCEEDED(Result) && NtWaitForSingleObject(Job->StopEvent, FALSE, &ZeroTimeout) == STATUS_TIMEOUT)
    {
        Result = ZpWindowShared_NextSample(Capture, 1000, &Sample, &Timestamp, NULL);
    ...
    _Out_ PULONG ResponseLength)
{
    ...
    if (OperationId == ZP_RECORDING_OPERATION_QUERY_CAPABILITIES)
    {
    ...
    else if (OperationId == ZP_RECORDING_OPERATION_START)
    {
    ...
    else if (OperationId == ZP_RECORDING_OPERATION_ENUMERATE)
    {
        Status = RequestLength == 0 ? ZpRecording_EncodeJobs(Client, NULL, Response, ResponseLength) :
                                      STATUS_INVALID_PARAMETER;
        return ZpStatus_FromNtStatus(Status);
    }
    else if (OperationId != ZP_RECORDING_OPERATION_STOP && OperationId != ZP_RECORDING_OPERATION_DELETE)
    {
        return ZpStatus_FromNtStatus(STATUS_NOT_SUPPORTED);
    }
