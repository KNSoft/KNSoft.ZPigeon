...
internal static class FirmwareWebApi
{
    ...
    internal static void MapFirmwareApi(this WebApplication app, NativeServer server)
    {
        ...
        app.MapPost("/api/firmware/variables", () =>
            server.EnumerateAdministrationAsync(AdministrationOperation.EnumerateFirmwareVariables));
        app.MapPost("/api/firmware/cpuid", async () =>
        {
            var data = await server.QueryAdministrationDataAsync(AdministrationOperation.QueryFirmwareData, "cpuid");
        ...
        app.MapPost("/api/firmware/smbios/structure", async (FirmwareSmbiosRequest request) =>
        {
            ...
            var data = await server.QueryAdministrationDataAsync(
                AdministrationOperation.QueryFirmwareData,
                request.Identity);
            if (data.Length > MaximumFirmwareDataLength)
            {
                return InvalidData("SMBIOS Structure 响应格式无效");
            }
            var status = SmbiosParser.TryParse(
                data,
                new SmbiosVersion(request.MajorVersion, request.MinorVersion, request.DmiRevision),
                out var table);
        ...
        app.MapPost("/api/firmware/data", async (HttpContext context, FirmwareDataRequest request) =>
        {
            ...
            var data = await server.QueryAdministrationDataAsync(
                AdministrationOperation.QueryFirmwareData,
                request.Identity);
            if (baseOffset > (ulong)data.Length ||
            ...
            context.Response.Headers["X-ZPigeon-Size"] = viewLength.ToString(CultureInfo.InvariantCulture);
            return Results.Bytes(
                data.AsSpan((int)(baseOffset + offset), (int)length).ToArray(),
                "application/octet-stream");
        });
        app.MapPost("/api/firmware/variable/write", async (HttpRequest request) =>
        {
            var (metadata, data) = await BinaryWebApi.ReadAsync<FirmwareVariableWriteRequest>(
                request,
                MaximumVariableLength);
            if (metadata.Action is not (AdministrationAction.Create or AdministrationAction.Configure or
                    AdministrationAction.Delete) ||
                (metadata.Action == AdministrationAction.Delete && data.Length != 0))
            {
                return Results.BadRequest();
            }
            string identity;
            if (metadata.Action == AdministrationAction.Create)
            {
                if (string.IsNullOrEmpty(metadata.Name) || metadata.Name.Contains('\0') ||
                    !Guid.TryParse(metadata.VendorGuid, out var vendorGuid))
                {
                    return Results.BadRequest();
                }
                identity = $"uefi:{{{vendorGuid:D}}}:{metadata.Name}";
            }
            else
            {
                if (string.IsNullOrEmpty(metadata.Identity) ||
                    !metadata.Identity.StartsWith("uefi:", StringComparison.Ordinal))
                {
                    return Results.BadRequest();
                }
            ...
            await server.ControlAdministrationDataAsync(
                AdministrationOperation.ControlFirmwareData,
                metadata.Action,
                metadata.Attributes,
                identity,
                data);
        ...
        app.MapPost("/api/firmware/variable/range/write", async (HttpRequest request) =>
        {
            var (metadata, rangeData) = await BinaryWebApi.ReadAsync<FirmwareVariableRangeWriteRequest>(
                request,
                (int)MaximumRangeLength);
            if (!metadata.Identity.StartsWith("uefi:", StringComparison.Ordinal) ||
                !ulong.TryParse(metadata.Offset, out var offset) || rangeData.Length == 0)
            {
                return Results.BadRequest();
            }
            var data = await server.QueryAdministrationDataAsync(
                AdministrationOperation.QueryFirmwareData,
                metadata.Identity);
            if (offset > (ulong)data.Length || rangeData.Length > data.Length - (int)offset)
            {
                return Results.BadRequest();
            }
            rangeData.CopyTo(data.AsSpan((int)offset));
            await server.ControlAdministrationDataAsync(
                AdministrationOperation.ControlFirmwareData,
        ...
        byte[] data)
    {
        var signature = identity.StartsWith("acpi:", StringComparison.Ordinal) && identity.Length > "acpi:".Length
            ...
            : identity;
        return new(identity, signature, "—", "—", "—", 0, (uint)data.Length, status.ToString(), [], true);
    }
...
internal sealed record FirmwareSmbiosRequest(
    string Identity,
    byte MajorVersion,
    byte MinorVersion,
    byte DmiRevision);
