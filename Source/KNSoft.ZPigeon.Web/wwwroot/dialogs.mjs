...
const negative = (button) =>
  negativeValue.test(button.value) ||
  negativeAction.test(button.dataset.action || "") ||
  ["取消", "关闭", "否"].includes(button.textContent.trim());
const dangerous = (button) =>
  button.classList.contains("danger") ||
  /(?:^|-)(?:delete|remove|clear|format|uninstall)(?:-|$)/i.test(button.dataset.action || "");

function defaultButton(dialog) {
  const values = buttons(dialog),
    explicit = values.find((button) => button.dataset.default !== undefined);
  ...
  const positive = values.find(
    (button) =>
      !dangerous(button) &&
      (positiveValue.test(button.value) ||
        positiveAction.test(button.dataset.action || "") ||
        positiveText.has(button.textContent.trim())),
  );
