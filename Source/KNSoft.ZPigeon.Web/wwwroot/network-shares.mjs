...
const shareType = (value) => {
  ...
  const name =
    type === 0 ? "文件夹" : type === 1 ? "打印机" : type === 2 ? "设备" : type === 3 ? "IPC" : `类型 ${type}`;
  return `${name}${value & 0x80000000 ? t("common.specialSuffix") : ""}` +
    `${value & 0x40000000 ? t("common.temporarySuffix") : ""}`;
};

const useType = (value) =>
  value === 0 ? "通配" : value === 1 ? "磁盘" : value === 2 ? "打印机" : value === 3 ? "IPC" : `类型 ${value}`;
const useState = (value) =>
  ["已连接", "已暂停", "会话丢失", "已断开", "网络错误", "正在连接", "正在重连"][value] || `状态 ${value}`;
...
export class NetworkShareManager {
    ...
    root.innerHTML = /* HTML */ ` <div class="manager-toolbar">
      ...
      <dialog data-role="connection">
        <form>
          ...
          <p class="property-note">连接属于当前账户。修改现有连接会先断开，再按新设置重新连接。</p>
          <div class="dialog-actions">
  ...
  render() {
    const loaded = this.records.has(this.view),
  ...
  row(record) {
    const row = document.createElement("tr"),
      published = record.kind === PUBLISHED;
    const values = published
      ? [
          record.identity,
          record.name,
          shareType(record.flags),
          record.state,
          Number(record.value) === UNLIMITED ? "无限制" : record.value,
          record.description,
        ]
      : [
          record.detail || "",
          record.name,
          record.description,
          useState(record.state),
          useType(record.flags),
          record.value,
        ];
    row.record = record;
  ...
  openMenu(event, record) {
    const actions =
      record.kind === PUBLISHED
        ? [
            ["编辑", () => this.edit(record)],
            ["权限", () => this.permissions(record)],
            ["删除", () => this.removePublished(record), true],
            ["属性", () => this.properties(record)],
          ]
        : [
            ["编辑/重新连接", () => this.edit(record)],
            ["断开", () => this.disconnectConnection(record), true],
            ["属性", () => this.properties(record)],
          ];
  ...
  edit(record = null) {
    if (this.view === "published") {
      ...
      const name = form.querySelector("[data-field=name]"),
        path = form.querySelector("[data-field=path]");
    ...
    try {
      await this.aclEditor.open({
        ...
        load: async () => {
          ...
          return { sddl: records[0].detail, daclProtected: !!(records[0].flags & 1) };
        },
        save: (sddl, daclProtected) =>
          this.call("/api/network-shares/published/security/set", {
  ...
  properties(record) {
    ...
    const fields = published
      ? [
          ["共享名称", record.identity],
          ["本地路径", record.name],
          ["类型", shareType(record.flags)],
      ...
      : [
          ["本地设备", record.detail || "—"],
          ["远程路径", record.name],
          ["用户名", record.description || "当前身份"],
          ["状态", useState(record.state)],
          ["类型", useType(record.flags)],
          ["使用计数", record.value],
        ];
