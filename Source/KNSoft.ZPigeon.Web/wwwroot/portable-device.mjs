...
export class PortableDeviceManager {
    ...
    host.innerHTML = `
      ...
      <div class="portable-body">
        ...
        <div class="manager-table">
          <table>
            <thead><tr><th>名称</th><th>类型</th><th>大小</th><th>修改时间</th><th>可用空间</th></tr></thead>
            <tbody></tbody>
          </table>
  ...
  renderDevices() {
    this.devicesHost.replaceChildren(...this.devices.map(device => {
  ...
  render() {
    this.body.replaceChildren(...this.records.map(item => {
  ...
  download(item) {
    ...
    link.href = `/api/portable/download?${query}`;
    link.click();
  }
  ...
  async upload() {
    ...
    try {
      ...
      const response = await fetch(`/api/portable/upload?${query}`, { method: 'PUT', body: file });
