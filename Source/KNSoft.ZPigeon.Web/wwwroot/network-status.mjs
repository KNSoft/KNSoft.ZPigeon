...
const operStatus = (value) =>
  ["", "已连接", "未连接", "测试中", "未知", "休眠", "不存在", "下层未连接"][value] || `状态 ${value}`;
...
class NetworkSnapshotManager {
  ...
  async load() {
    ...
    try {
      const records = await this.call(this.endpoint);
...
export class NetworkAdapterManager extends NetworkSnapshotManager {
  ...
  render() {
    const all = (this.records || []).filter((record) => record.kind === ADAPTER),
      visible = all.filter((record) => this.showHidden.checked || !(record.state & 0x80000000));
  ...
  row(record) {
    const row = document.createElement("tr"),
      admin = (record.state >>> 16) & 0xffff,
      addresses = this.addresses(record.identity),
      category = Number((record.detail || "").split("\n")[10]);
    const values = [
      record.name,
      admin === 2 ? "已禁用" : operStatus(record.state & 0xffff),
      category === 0 ? "公用网络" : category === 1 ? "专用网络" : category === 2 ? "域网络" : "—",
      adapterType(record.flags),
      addresses.map((address) => `${address.name}/${address.value}`).join(", "),
      formatBits(record.value),
      record.description,
    ];
    row.record = record;
  ...
  openMenu(event, record) {
    ...
    const category = Number((record.detail || "").split("\n")[10]),
      actions = [
        [disabled ? "启用" : "禁用", () => this.setEnabled(record, disabled)],
        [
          "设为公用网络",
          () => this.setCategory(record, 0),
          false,
          category === 0 || category === 2 || !Number.isInteger(category),
        ],
        [
          "设为专用网络",
          () => this.setCategory(record, 1),
          false,
          category === 1 || category === 2 || !Number.isInteger(category),
        ],
        ["属性", () => this.properties(record)],
      ];
    this.menu.replaceChildren(
      ...actions.map(([name, action, danger, actionDisabled]) => {
  ...
  async setCategory(record, category) {
    try {
      await this.call("/api/network-adapters/control", {
        action: 23,
        identity: record.identity,
        argument: String(category),
      });
      this.notify(category ? "已设为专用网络" : "已设为公用网络");
      await this.load();
    } catch (error) {
      this.notify(error);
    }
  ...
  properties(record) {
    ...
    const ipv4 =
      addresses
        .filter((item) => item.state === IPV4)
        .map((item) => `${item.name}/${item.value}`)
        .join("\n") || "—";
    const ipv6 =
      addresses
        .filter((item) => item.state === IPV6)
        .map((item) => `${item.name}/${item.value}`)
        .join("\n") || "—";
    const fields = [
      ["名称", record.name],
      ["描述", record.description],
      ["接口索引", record.identity],
      ["管理状态", ((record.state >>> 16) & 0xffff) === 2 ? "已禁用" : "已启用"],
      ["运行状态", operStatus(record.state & 0xffff)],
      ["类型", adapterType(record.flags)],
      ["MAC 地址", detail[0] || "—"],
      ["MTU", detail[1] || "—"],
      ["发送速度", formatBits(record.value)],
      ["接收速度", formatBits(detail[2])],
      ["接收流量", formatBytes(detail[3])],
      ["发送流量", formatBytes(detail[4])],
      ["接收错误", detail[5] || "0"],
      ["发送错误", detail[6] || "0"],
      ["发送队列", detail[7] || "0"],
      ["IPv4", ipv4],
    ...
    this.dialog.querySelector("dl").replaceChildren(
...
export class NetworkRouteManager extends NetworkSnapshotManager {
  constructor(root, { call, notify }) {
    ...
    root.innerHTML = /* HTML */ ` <div class="manager-toolbar">
      ...
      <div class="context-menu administration-menu" data-role="menu" hidden>
        <button data-action="edit">修改跃点数</button><button data-action="delete" class="danger">删除</button>
      </div>
      <dialog data-role="route">
        <form>
          <h2 data-role="title"></h2>
          <label
            >地址族<select data-field="family">
              <option value="2">IPv4</option>
              <option value="23">IPv6</option>
            </select></label
          ><label>目标前缀<input data-field="destination" required placeholder="0.0.0.0/0" /></label
          ><label>下一跃点<input data-field="next-hop" required placeholder="0.0.0.0" /></label
          ><label>接口索引<input data-field="interface" type="number" min="1" required /></label
          ><label>跃点数<input data-field="metric" type="number" min="0" required /></label>
  ...
  render() {
    ...
    const records = this.visible().sort(
      (left, right) => left.state - right.state || left.name.localeCompare(right.name),
    );
    this.body.replaceChildren(
      ...records.map((record) => {
        const row = document.createElement("tr"),
          protocol = record.flags & 0xffff,
          origin = record.flags >>> 16,
          index = record.identity.split("|")[1] || "";
        row.record = record;
        const values = [
          record.name,
          record.description,
  ...
  edit(record = null) {
    ...
    const field = (name) => this.dialog.querySelector(`[data-field="${name}"]`),
      parts = record?.identity.split("|");
    this.dialog.querySelector("[data-role=title]").textContent = record ? "修改路由" : "新建路由";
    field("family").value = String(record?.state || 2);
    field("destination").value = record?.name || (record?.state === IPV6 ? "::/0" : "0.0.0.0/0");
    field("next-hop").value = record?.description || (record?.state === IPV6 ? "::" : "0.0.0.0");
    field("interface").value = parts?.[1] || "";
    field("metric").value = record?.value || 0;
    for (const name of ["family", "destination", "next-hop", "interface"]) field(name).disabled = !!record;
    this.dialog.showModal();
    field(record ? "metric" : "destination").focus();
  }

  async save() {
    const field = (name) => this.dialog.querySelector(`[data-field="${name}"]`).value,
      identity =
        this.editing?.identity ||
        `${field("family")}|${field("interface")}|${field("destination")}|${field("next-hop")}`;
    try {
      await this.call("/api/network-routes/control", {
        action: this.editing ? 23 : 1,
        identity,
        argument: field("metric"),
      });
      this.dialog.close();
      this.notify("路由已保存");
      await this.load();
  ...
  async remove(record) {
    if (!confirm(`确定删除路由 ${record.name}？`)) return;
    try {
      await this.call("/api/network-routes/control", { action: 2, identity: record.identity });
      this.notify("路由已删除");
      await this.load();
    } catch (error) {
      this.notify(error);
...
export class NetworkEndpointManager extends NetworkSnapshotManager {
  constructor(root, { call, notify, revealProcess }) {
    ...
    root.innerHTML = /* HTML */ ` <div class="manager-toolbar">
      ...
      <div class="context-menu administration-menu" data-role="menu" hidden>
        <button data-action="process">转到进程</button><button data-action="close" class="danger">关闭 TCP 连接</button>
      </div>`;
    this.filter = root.querySelector("[data-role=filter]");
    ...
    this.body.oncontextmenu = (event) => {
      ...
      event.preventDefault();
      this.selected = row.record;
      this.menu.querySelector("[data-action=process]").disabled = !Number(this.selected.value);
      this.menu.querySelector("[data-action=close]").disabled =
        this.selected.kind !== TCP || this.selected.flags !== IPV4 || this.selected.state !== 5;
      this.menu.hidden = false;
      const box = this.menu.getBoundingClientRect();
      this.menu.style.left = `${Math.max(6, Math.min(event.clientX, innerWidth - box.width - 6))}px`;
      this.menu.style.top = `${Math.max(6, Math.min(event.clientY, innerHeight - box.height - 6))}px`;
    };
    this.menu.querySelector("[data-action=process]").onclick = () => {
      this.menu.hidden = true;
      this.revealProcess?.(Number(this.selected?.value));
    };
    this.menu.querySelector("[data-action=close]").onclick = () => this.closeConnection();
    addEventListener("pointerdown", (event) => {
      if (!this.menu.contains(event.target)) this.menu.hidden = true;
    });
    this.render();
  ...
  render() {
    const all = this.records || [],
      selection = this.protocol.value;
    const records = this.visible().filter(
      (record) =>
        selection === "all" ||
        (selection === "tcp" && record.kind === TCP) ||
        (selection === "udp" && record.kind !== TCP) ||
        (selection === "ipv4" && record.flags === IPV4) ||
        (selection === "ipv6" && record.flags === IPV6),
    );
    records.sort((left, right) => left.kind - right.kind || left.name.localeCompare(right.name));
    this.body.replaceChildren(
      ...records.map((record) => {
        const tcp = record.kind === TCP,
          pid = Number(record.value) || 0,
          row = document.createElement("tr");
        row.record = record;
        const values = [
          `${tcp ? "TCP" : "UDP"}${record.flags === IPV4 ? "v4" : "v6"}`,
          record.name,
          record.description || "—",
          tcp ? tcpState(record.state) : "—",
          pid || "-",
        ];
        for (const [index, value] of values.entries()) {
          ...
          cell.textContent = value;
          cell.title = cell.textContent;
          if (index === 1 || index === 2 || (index === 4 && pid)) cell.dataset.copyable = "";
        }
        return row;
      }),
    );
  ...
  async closeConnection() {
    ...
    this.menu.hidden = true;
    if (!record || record.kind !== TCP || record.flags !== IPV4 || record.state !== 5) return;
    if (!confirm(t("network.confirmClose", { local: record.name, remote: record.description }))) return;
    try {
      await this.call("/api/network-endpoints/control", {
        action: 7,
