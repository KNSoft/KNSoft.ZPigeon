export function bindTerminalInteractions(session, onPaste, onError) {
  ...
  terminal.attachCustomKeyEventHandler((event) => {
    ...
    const key = event.key.toLowerCase(),
      ctrl = event.ctrlKey && !event.altKey && !event.metaKey;
    if (ctrl && key === "insert") {
      void copy();
      return false;
    }
    if ((ctrl && key === "v") || (event.shiftKey && !event.ctrlKey && key === "insert")) {
      void paste();
      return false;
    }
    return true;
  ...
  host.addEventListener(
    ...
    (event) => {
      if (event.ctrlKey && !event.altKey && !event.metaKey && event.key.toLowerCase() === "c") {
        ...
        if (terminal.hasSelection() || event.shiftKey) void copy();
        else session.send("\x03");
      }
    },
    true,
  );
...
export function exportTerminal(session) {
  ...
  const stamp = new Date()
      ...
      .replace(/[-:]/g, "")
      .replace(/\.\d{3}Z$/, "Z"),
    url = URL.createObjectURL(new Blob([getTerminalText(session.terminal)], { type: "text/plain;charset=utf-8" })),
