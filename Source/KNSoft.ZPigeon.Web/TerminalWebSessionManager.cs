...
internal sealed class TerminalWebSessionManager : IAsyncDisposable
{
    ...
    {
        var terminal = await server.CreateShellAsync(shell, columns, rows, workingDirectory);
    ...
    internal async Task<TerminalWebSessionInfo> CreateExecutionAsync(
        ...
        string? cleanupPath)
    {
        var terminal = await server.CreateTerminalAsync(start, columns, rows, title);
        return Add(terminal, 0, cleanupPath is null ? null : () => server.DeleteFileAsync(cleanupPath));
    }
