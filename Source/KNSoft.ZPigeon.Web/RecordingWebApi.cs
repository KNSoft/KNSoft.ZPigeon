...
{
    internal static void MapRecordingApi(this WebApplication app, ClientServicesRegistry services)
    {
        ...
        app.MapPost("/api/recording/stop", (RecordingIdRequest request) =>
            server.StopRecordingAsync(request.RecordingId));
        app.MapPost("/api/recording/delete", (RecordingIdRequest request) =>
            server.DeleteRecordingAsync(request.RecordingId));
    }
    ...
    private static object ToWebRecord(RecordingRecord record) => new
    {
        ...
        record.State,
        record.Status,
        StartTime = record.StartTime.ToString(CultureInfo.InvariantCulture),
        Duration = record.Duration.ToString(CultureInfo.InvariantCulture),
        FileSize = record.FileSize.ToString(CultureInfo.InvariantCulture),
        record.Path
    };
}
