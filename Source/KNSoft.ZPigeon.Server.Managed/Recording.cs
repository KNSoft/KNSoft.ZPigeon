...
public enum RecordingSource : byte
{
    AudioOutput = 1,
    AudioInput,
    Camera,
...
public enum RecordingCodec : byte
{
    Auto = 1,
    Pcm,
    Aac,
...
public enum RecordingAudioSource : byte
{
    None,
    Output,
    Input
...
[Flags]
public enum RecordingFlags : byte
{
    CaptureCursor = 1
}
...
public sealed partial class NativeServer
{
    ...
    public Task<uint> QueryRecordingCapabilitiesAsync() =>
        RunManagementAsync<uint>(context => NativeMethods.QueryRecordingCapabilities(ClientId,
            RecordingCapabilitiesCallback,
            context));

    public async Task<RecordingRecord> StartRecordingAsync(RecordingOptions options)
    {
        var records = await RunManagementAsync<RecordingRecord[]>(context => NativeMethods.StartRecording(ClientId,
            options.Source,
            options.Codec,
            options.FrameRate,
            options.AudioSource,
    ...
    public Task<RecordingRecord[]> EnumerateRecordingsAsync() =>
        RunManagementAsync<RecordingRecord[]>(context => NativeMethods.EnumerateRecordings(ClientId,
            RecordingRecordsCallback,
            context));

    public Task StopRecordingAsync(uint recordingId) =>
        RunStatusAsync((callback, context) => NativeMethods.StopRecording(ClientId, recordingId, callback, context));

    public Task DeleteRecordingAsync(uint recordingId) =>
        RunStatusAsync((callback, context) => NativeMethods.DeleteRecording(ClientId, recordingId, callback, context));
...
internal static partial class NativeMethods
{
    ...
    internal static partial int QueryRecordingCapabilities(
        ulong clientId,
        RecordingCapabilitiesCallback callback,
        nint context);
    ...
    internal static partial int EnumerateRecordings(ulong clientId, RecordingRecordsCallback callback, nint context);

    [LibraryImport(Library, EntryPoint = "ZpNative_StopRecording")]
    internal static partial int StopRecording(ulong clientId, uint recordingId, StatusCallback callback, nint context);

    [LibraryImport(Library, EntryPoint = "ZpNative_DeleteRecording")]
    internal static partial int DeleteRecording(
        ulong clientId,
        uint recordingId,
