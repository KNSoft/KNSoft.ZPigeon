...
{
    private const byte TunnelProtocolTcp = 1;
    private const byte TunnelProtocolUdp = 2;
    ...
    private Task<RemoteTunnel> OpenTunnelAsync(string host, ushort port, byte protocol)
    {
        ...
        var status = NativeMethods.OpenTunnel(ClientId, host,
                                              (uint)host.Length,
                                              port,
                                              protocol,
                                              TunnelOpenCallback,
...
public sealed class RemoteTunnel : IAsyncDisposable
{
    ...
    private readonly Channel<OwnedBuffer> output =
        Channel.CreateBounded<OwnedBuffer>(new BoundedChannelOptions(16)
        {
            SingleReader = true,
            SingleWriter = true,
    ...
    internal bool Receive(nint data, uint dataLength)
    {
        ...
        var buffer = OwnedBuffer.Copy(data + offset, (int)dataLength - offset);
        try
        {
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        output.Writer.TryComplete();
        _ = NativeMethods.CloseTunnel(tunnel);
