...
public sealed partial class NativeServer
{
    ...
    public Task<WmiRow[]> EnumerateWmiNamespacesAsync(string wmiNamespace) =>
        RunManagementAsync<WmiRow[]>(context => NativeMethods.EnumerateWmiNamespaces(
            wmiNamespace,
    ...
    public Task<WmiRow[]> EnumerateWmiClassesAsync(string wmiNamespace) =>
        RunManagementAsync<WmiRow[]>(context => NativeMethods.EnumerateWmiClasses(
            wmiNamespace,
            (uint)wmiNamespace.Length,
            WmiCallback,
            context));
    ...
    public Task<WmiRow[]> QueryWmiAsync(
        ...
        RunManagementAsync<WmiRow[]>(context => NativeMethods.QueryWmi(
            wmiNamespace,
            (uint)wmiNamespace.Length,
            query,
            (uint)query.Length,
        ...
        for (var rowIndex = 0; rowIndex < result.Length; rowIndex++)
        {
            ...
            for (var cellIndex = 0; cellIndex < cells.Length; cellIndex++)
            {
                ...
                cells[cellIndex] = new WmiCell(
                    cell.Type,
                    Marshal.PtrToStringUni(cell.Name, (int)cell.NameLength) ?? string.Empty,
                    Marshal.PtrToStringUni(cell.Value, (int)cell.ValueLength) ?? string.Empty);
            }
