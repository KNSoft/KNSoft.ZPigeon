...
public sealed partial class NativeServer
{
    ...
    private Task<FileTransfer> OpenFileAsync(string path, ulong fileSize, bool overwrite, bool write)
    {
        ...
        var status = write ?
            NativeMethods.OpenFileWrite(ClientId, path,
                                        (uint)path.Length,
                                        fileSize,
                                        overwrite,
                                        FileOpenCallback,
                                        ...
                                        FileCloseCallback,
                                        context) :
            NativeMethods.OpenFileRead(ClientId, path,
                                       (uint)path.Length,
                                       fileSize,
                                       FileOpenCallback,
                                       FileDataCallback,
...
public sealed class FileTransfer : IAsyncDisposable
{
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        output.Writer.TryComplete();
        _ = NativeMethods.CloseFileTransfer(transfer);
