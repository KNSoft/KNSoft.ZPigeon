...
TEST_FUNC(ProtocolMessage)
{
    ...
    ZP_SERVICE_CONTROL ServiceControl;
    ...
    ZP_EXECUTION_RUNTIME_RECORD ExecutionRuntimes[] = {
        { ZpExecutionRuntimeCommandPrompt,
          { IMAGE_FILE_MACHINE_AMD64, IMAGE_SUBSYSTEM_WINDOWS_CUI, { 10, 0, 26100, 1 } },
          L"C:\\Windows\\System32\\cmd.exe", 27 },
        { ZpExecutionRuntimeNode,
          { IMAGE_FILE_MACHINE_AMD64, IMAGE_SUBSYSTEM_WINDOWS_CUI, { 24, 18, 0, 0 } },
          L"C:\\Node\\node.exe", 16 }
    };
    ZP_EXECUTION_ENVIRONMENT_VIEW ExecutionEnvironment;
    ZP_EXECUTION_RUNTIME_RECORD_VIEW ExecutionRuntime;
    ZP_EXECUTION_IMAGE_INFO ExecutionImage = {
        IMAGE_FILE_MACHINE_AMD64, IMAGE_SUBSYSTEM_WINDOWS_GUI, { 1, 2, 3, 4 }
    };
    ZP_EXECUTION_IMAGE_INFO ExecutionImageView;
    ZP_EXECUTION_START ExecutionStart = {
        ZpExecutionEngineCreateProcess,
        ZpExecutionIdentityCustomToken,
        ZP_EXECUTION_SESSION_CURRENT,
        ZP_EXECUTION_FLAG_HIDDEN | ZP_EXECUTION_FLAG_JOB_OBJECT,
        L"cmd.exe", 7,
        L"/Q", 2,
        L"C:\\", 3,
        NULL, 0,
        ...
        NULL, 0,
        NULL, 0,
        CustomToken, sizeof(CustomToken)
    };
    ZP_EXECUTION_START_VIEW ExecutionStartView;
    ZP_TERMINAL_CREATE_VIEW TerminalCreate;
    ZP_EXECUTION_START TerminalStart = {
        ZpExecutionEngineCreateProcess,
        ZpExecutionIdentityCurrent,
        ZP_EXECUTION_SESSION_CURRENT,
        ZP_EXECUTION_FLAG_HIDDEN,
        L"cmd.exe", 7,
        L"/Q", 2,
        L"C:\\", 3
    };
    ...
    ULONG Length, Index, ExitCode, CreditBytes, ProcessId, FileAttributes;
    BYTE Shells;
    ...
    ULONG WmiRowOffset = 0, WmiCellOffset = 0, ExecutionRuntimeOffset = 0;
    ...
    TEST_OK(NT_SUCCESS(ZpProcess_EncodeControl(ProcessInfo.ProcessId,
                                               ...
                                               sizeof(Buffer),
                                               &Length)) &&
            Length == 2 * sizeof(ULONG) + sizeof(ULONGLONG) + sizeof(BYTE) &&
    ...
    TEST_OK(ZpFile_EncodeEnumeratePageRequest(L"C:\\Test",
                                               ...
                                               sizeof(Buffer),
                                               &Length) == STATUS_INVALID_PARAMETER);
    TEST_OK(NT_SUCCESS(ZpFile_EncodeFilteredPageRequest(L"C:\\Test",
                                                        7,
                                                        L"*.txt",
                                                        5,
                                                        L'A',
                                                        ...
                                                        sizeof(Buffer),
                                                        &Length)) &&
            NT_SUCCESS(ZpFile_DecodeFilteredPageRequest(Buffer,
                                                        Length,
                                                        &FilePathView,
                                                        &FileFilterView,
                                                        &FileGroup,
                                                        &EnumerationId)) &&
            FilePathView.Length == 7 &&
            FileFilterView.Length == 5 &&
            FileGroup == L'A' &&
            EnumerationId == 0 &&
            NT_SUCCESS(ZpFile_EncodeFilteredPageRequest(NULL,
                                                        0,
                                                        NULL,
                                                        0,
                                                        UNICODE_NULL,
            ...
            NT_SUCCESS(ZpFile_DecodeFilteredPageRequest(Buffer,
                                                        ...
                                                        &EnumerationId)) &&
            FilePathView.Length == 0 &&
            FileFilterView.Length == 0 &&
            FileGroup == UNICODE_NULL &&
            EnumerationId == 42);
    TEST_OK(ZpFile_EncodeFilteredPageRequest(L"C:\\Test",
                                              7,
                                              NULL,
                                              0,
                                              L'a',
                                              ...
                                              sizeof(Buffer),
                                              &Length) == STATUS_INVALID_PARAMETER &&
            ZpFile_EncodeFilteredPageRequest(L"C:\\Test",
                                              7,
                                              NULL,
                                              0,
                                              L'#',
    ...
    TEST_OK(NT_SUCCESS(ZpAdministration_EncodeList(
            ...
            AdministrationRecord.Kind == ZpAdministrationKindSecurityDescriptor &&
            ...
            AdministrationRecord.Data.Length == sizeof(AdministrationData) &&
    ...
    TEST_OK(NT_SUCCESS(ZpAdministration_EncodeControl(ZpAdministrationActionSetDefault,
                                                       L"TIP",
                                                       3,
                                                       NULL,
                                                       0,
    ...
    TEST_OK(NT_SUCCESS(ZpRegistry_EncodeSecurityRequest(
            ...
            RegistrySecurityRequest.Sddl.Length ==
                ARRAYSIZE(RegistrySddl) - 1 &&
            RegistrySecurityRequest.DaclProtected);
    TEST_OK(NT_SUCCESS(ZpExecution_EncodeEnvironment(
                           ZP_EXECUTION_ENVIRONMENT_FLAG_ADMINISTRATOR,
                           ExecutionRuntimes,
                           ARRAYSIZE(ExecutionRuntimes),
                           Buffer,
                           sizeof(Buffer),
                           &Length)) &&
            NT_SUCCESS(ZpExecution_DecodeEnvironment(Buffer, Length, &ExecutionEnvironment)) &&
            ExecutionEnvironment.Flags == ZP_EXECUTION_ENVIRONMENT_FLAG_ADMINISTRATOR &&
            ExecutionEnvironment.Count == ARRAYSIZE(ExecutionRuntimes) &&
            NT_SUCCESS(ZpExecution_GetNextRuntime(&ExecutionEnvironment,
                                                   &ExecutionRuntimeOffset,
                                                   &ExecutionRuntime)) &&
            ExecutionRuntime.Kind == ZpExecutionRuntimeCommandPrompt &&
            ExecutionRuntime.Image.Subsystem == IMAGE_SUBSYSTEM_WINDOWS_CUI &&
            ExecutionRuntime.Image.Version[2] == 26100 && ExecutionRuntime.Path.Length == 27 &&
            NT_SUCCESS(ZpExecution_GetNextRuntime(&ExecutionEnvironment,
                                                   &ExecutionRuntimeOffset,
                                                   &ExecutionRuntime)) &&
            ExecutionRuntime.Kind == ZpExecutionRuntimeNode && ExecutionRuntime.Path.Length == 16 &&
            ZpExecution_GetNextRuntime(&ExecutionEnvironment,
                                       &ExecutionRuntimeOffset,
                                       &ExecutionRuntime) == STATUS_INVALID_PARAMETER);
    TEST_OK(NT_SUCCESS(ZpExecution_EncodeImageInfo(&ExecutionImage,
                                                    Buffer,
                                                    sizeof(Buffer),
                                                    &Length)) &&
            NT_SUCCESS(ZpExecution_DecodeImageInfo(Buffer, Length, &ExecutionImageView)) &&
            ExecutionImageView.Machine == IMAGE_FILE_MACHINE_AMD64 &&
            ExecutionImageView.Subsystem == IMAGE_SUBSYSTEM_WINDOWS_GUI &&
            ExecutionImageView.Version[3] == 4 &&
            ZpExecution_DecodeImageInfo(Buffer, Length - 1, &ExecutionImageView) == STATUS_DATA_ERROR);
    TEST_OK(NT_SUCCESS(ZpExecution_EncodeStart(&ExecutionStart,
                                               Buffer,
                                               sizeof(Buffer),
                                               &Length)) &&
            NT_SUCCESS(ZpExecution_DecodeStart(Buffer, Length, &ExecutionStartView)) &&
            ExecutionStartView.Identity == ZpExecutionIdentityCustomToken &&
            FlagOn(ExecutionStartView.Flags, ZP_EXECUTION_FLAG_JOB_OBJECT) &&
            ExecutionStartView.FileName.Length == 7 && ExecutionStartView.Arguments.Length == 2 &&
            ExecutionStartView.CustomToken.Length == sizeof(CustomToken) &&
            RtlCompareMemory(ExecutionStartView.CustomToken.Buffer,
                             CustomToken,
                             sizeof(CustomToken)) == sizeof(CustomToken));
    TEST_OK(NT_SUCCESS(ZpTerminal_EncodeCreate(120,
            ...
            TerminalCreate.Start.FileName.Length == 7 &&
            TerminalCreate.Start.Arguments.Length == 2 &&
            TerminalCreate.Start.WorkingDirectory.Length == 3);
