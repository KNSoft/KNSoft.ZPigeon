...
static const SOCKADDR_IN SDKTestRemoteAddress = { AF_INET };
...
typedef struct _SDK_SYSTEM_LOOPBACK
{
    ...
    ULONGLONG CurrentProcessCreateTime;
    ...
    _In_ ULONG PayloadLength)
{
    ...
    if (MessageType == ZpMessageChannelData)
    {
        ...
        {
            if (Body == NULL ||
                BodyLength != sizeof(ChannelData.ChannelId) ||
                PayloadLength > ZP_CHANNEL_DATA_MAX_SIZE)
            {
                return STATUS_INVALID_BUFFER_SIZE;
            }
            RtlCopyMemory(&ChannelData.ChannelId, Body, sizeof(ChannelData.ChannelId));
            TestContext->SendChannelId = ChannelData.ChannelId;
            TestContext->SendChannelDataLength = PayloadLength;
    ...
    if (NT_SUCCESS(Status))
    {
        Request.Payload.Buffer = Payload;
        Request.Payload.Length = PayloadLength;
        if (RequestConnection->SendDelay != 0)
        {
...
SDKTest_OrderedConcurrentRequests(VOID)
{
    ...
    if (!NT_SUCCESS(ZpServerConnection_Initialize(
                        ...
                        1,
                        (const SOCKADDR*)&SDKTestRemoteAddress,
                        sizeof(SDKTestRemoteAddress),
                        SDKTest_RequestConnectionSend,
                        SDKTest_ConnectionDisconnect,
                        SDKTest_RequestConnectionDestroy)))
    {
...
SDKTest_SystemConnectionSend(
    ...
    _In_reads_bytes_opt_(BodyLength) const VOID* Body,
    _In_ ULONG BodyLength,
    _In_reads_bytes_opt_(RequestPayloadLength) const VOID* RequestPayload,
    _In_ ULONG RequestPayloadLength)
{
    PSDK_SYSTEM_LOOPBACK Loopback = CONTAINING_RECORD(Connection,
                                                      SDK_SYSTEM_LOOPBACK,
                                                      Connection);
    ...
    PZP_CLIENT_LOCAL_CHANNEL FileChannel = NULL;
    ZP_REQUEST_VIEW Request;
    ...
    Request.Payload.Buffer = RequestPayload;
    Request.Payload.Length = RequestPayloadLength;
    Loopback->SendCount++;
    ...
    {
        ResultStatus = ZpProcess_Execute(&Loopback->Client,
                                         Request.OperationId,
                                         Request.Payload.Buffer,
                                         Request.Payload.Length,
                                         &AllocatedPayload,
    ...
    if (Processes != NULL)
    {
        ...
        for (Index = 0; Index < Processes->Count; Index++)
        {
                ...
                Process.ProcessId == GetCurrentProcessId())
            {
                ...
                Loopback->CurrentProcessCreateTime = Process.CreateTime;
                break;
            }
        }
    }
...
VOID
NTAPI
SDKTest_ProcessModulesCallback(
    _In_ ZP_REQUEST_HANDLE Request,
    _In_ ZP_STATUS Status,
    ...
    _In_opt_ PVOID Context)
{
    ...
    if (Modules != NULL)
    {
        Loopback->ProcessModuleCount = Modules->Count;
        Loopback->ProcessMachineType = Modules->MachineType;
        Loopback->ProcessMachineBits = Modules->MachineBits;
        for (Index = 0; Index < Modules->Count; Index++)
        {
            if (NT_SUCCESS(ZpProcess_GetNextModule(Modules, &Offset, &Module)) &&
                Module.BaseAddress != 0 && Module.SizeOfImage != 0 &&
...
static
VOID
SDKTest_ClientLocalChannelCommit(
    _Inout_ PZP_CLIENT_LOCAL_CHANNEL Channel,
    _In_ LOGICAL ResponseSent)
{
    UNREFERENCED_PARAMETER(Channel);
    UNREFERENCED_PARAMETER(ResponseSent);
}
...
TEST_FUNC(SDKContract)
{
    ...
    PZP_CLIENT_LOCAL_CHANNEL ReferencedLocalChannel;
    ...
    static SDK_SYSTEM_LOOPBACK SystemLoopback;
    ...
    RtlInitializeSRWLock(&SystemLoopback.Client.ArchiveEnumerationLock);
    RtlInitializeSRWLock(&SystemLoopback.Client.SnapshotLock);
    InitializeListHead(&SystemLoopback.Client.FileEnumerations);
    InitializeListHead(&SystemLoopback.Client.ArchiveEnumerations);
    InitializeListHead(&SystemLoopback.Client.Snapshots);
    ...
    TEST_OK(NT_SUCCESS(ZpServer_EnumerateProcessModules(
                (ZP_CONNECTION_HANDLE)&SystemLoopback.Connection,
                GetCurrentProcessId(),
            ...
            ZpStatus_IsSuccess(SystemLoopback.Status) &&
            SystemLoopback.ProcessModuleCount != 0 &&
            SystemLoopback.ProcessMachineType != IMAGE_FILE_MACHINE_UNKNOWN &&
    ...
    TEST_OK(NT_SUCCESS(ZpClientLocalChannel_ReceiveClose(ClientObject,
                                                         &ChannelClose)) &&
            ClientObject->LocalChannelCount == 0);
    TEST_OK(ZpClientLocalChannel_ReferenceById(ClientObject,
                                               ChannelClose.ChannelId,
                                               0,
                                               &ReferencedLocalChannel) ==
                STATUS_INVALID_HANDLE &&
            NT_SUCCESS(ZpClientLocalChannel_ReceiveClose(ClientObject,
                                                         &ChannelClose)));
    RtlZeroMemory(&LocalChannel, sizeof(LocalChannel));
    TEST_OK(NT_SUCCESS(ZpClientLocalChannel_Insert(
                ClientObject,
            ...
            TestContext.SendChannelId == LocalChannel.ChannelId &&
            SDK_STATUS_IS(TestContext.SendChannelStatus,
                          STATUS_PROTOCOL_UNREACHABLE));
    ChannelData.ChannelId = ClientObject->NextLocalChannelId;
    TEST_OK(NT_SUCCESS(ZpClientLocalChannel_ReceiveData(ClientObject,
                                                        &ChannelData)) &&
            TestContext.SendMessageType == ZpMessageChannelClose &&
            TestContext.SendChannelId == ChannelData.ChannelId &&
    ...
    TEST_OK(TestContext.SendMessageType == ZpMessageCancel &&
    ...
    TEST_OK(NT_SUCCESS(ZpServerConnection_ReceiveChannelClose(
            ...
            SDK_STATUS_IS(TestContext.ChannelCloseStatus,
                          STATUS_PROTOCOL_UNREACHABLE));
    TEST_OK(NT_SUCCESS(ZpServerConnection_ReceiveChannelWindow(
                           &RegistryConnection.Connection,
                           4,
            ...
            ZpChannel_Send(TestContext.FileWriteChannel,
                           FileWriteData,
                           sizeof(FileWriteData)) == STATUS_INVALID_DEVICE_STATE);
    TEST_OK(NT_SUCCESS(ZpServerConnection_ReceiveChannelClose(
                           &RegistryConnection.Connection,
                           &ChannelClose)) &&
            TestContext.ChannelCloseCount == 4);
    ...
    TEST_OK(ZpStatus_IsSuccess(ZpServer_Start(Server)) &&
            ServerObject->State == ZpServerStateRunning &&
            TestContext.StartCount == 1 &&
            TestContext.ServerStateCount == 2 &&
