    ...
    _In_ ZP_TRANSPORT_TYPE Transport)
{
    ...
    static const WCHAR TerminalArguments[] = L"/D /Q";
    static const WCHAR TerminalCancelArguments[] =
        L"-NoLogo -NoProfile -Command \"Start-Sleep -Seconds 30\"";
    ...
    ZP_EXECUTION_START TerminalStart = {
        ...
        L"cmd.exe", 7,
        TerminalArguments, ARRAYSIZE(TerminalArguments) - 1
    };
    ZP_EXECUTION_START TerminalCancelStart = {
        ZpExecutionEngineCreateProcess,
        ZpExecutionIdentityCurrent,
        ZP_EXECUTION_SESSION_CURRENT,
        ZP_EXECUTION_FLAG_HIDDEN,
        L"powershell.exe", 14,
        TerminalCancelArguments, ARRAYSIZE(TerminalCancelArguments) - 1
    };
    ...
    static BYTE RegistryOversizedData[8192];
    static WCHAR FirstEventBookmark[4096];
    ULONG FirstEventBookmarkLength;
    static BYTE FileWriteData[0x20000 + 17];
    NTSTATUS Status;
    ...
    TestContext.SystemInfoEvent = Events[6];
    TestContext.ProcessListEvent = Events[7];
    TestContext.ProcessInfoEvent = Events[8];
    TestContext.ServiceListEvent = Events[9];
    TestContext.ServiceInfoEvent = Events[10];
    TestContext.ProcessTerminateEvent = Events[11];
    TestContext.ServiceControlEvent = Events[12];
    TestContext.FileInfoEvent = Events[13];
    TestContext.FilePageEvent = Events[14];
    TestContext.EventLogPageEvent = Events[15];
    ...
    TestContext.RegistryPageEvent = Events[22];
    TestContext.RegistryValueEvent = Events[23];
    TestContext.RegistryStatusEvent = Events[24];
    ...
    if (!NT_SUCCESS(Status) ||
        ...
        TestContext.FilePageCount > ZP_FILE_PAGE_COUNT)
    {
        goto Cleanup;
    }
    if (TestContext.FilePageEnumerationId != 0)
    {
        ...
        Status = ZpServer_EnumerateFilesPage(
            ...
            NULL,
            0,
            TestContext.FilePageEnumerationId,
            SDK_INTEGRATION_TIMEOUT_MILLISECONDS,
            SDKIntegration_FilePageCallback,
            &TestContext,
            &Request);
