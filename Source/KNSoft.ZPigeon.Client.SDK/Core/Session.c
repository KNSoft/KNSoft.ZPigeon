    ...
    _In_opt_ PVOID Context)
{
    ...
    switch (Frame->MessageType)
    {
        ...
        case ZpMessagePong:
            Status = ZpMessage_DecodePing(ZpMessagePong, Frame->Body, Frame->BodyLength, &Token);
            return NT_SUCCESS(Status) ?
                       ZpClient_NotifyPong((ZP_CLIENT_HANDLE)Session->Owner, Token) : Status;
    ...
    _Inout_ PZP_CLIENT_SESSION Session)
{
    ...
    return NT_SUCCESS(Status) ?
               Session->Send(Session->Context, ZpMessageClientHello, Body, BodyLength) : Status;
}
