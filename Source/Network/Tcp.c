...
ZpTcpConnection_AllocateSend(
    ...
    _In_opt_ PZP_CONNECTION ProtocolConnection)
{
    PZP_TCP_SEND_IO Send = Mem_Alloc(FIELD_OFFSET(ZP_TCP_SEND_IO, Data) + DataLength);

    if (Send != NULL)
    {
        Send->ProtocolConnection = ProtocolConnection;
        Send->EnqueuedTickCount = GetTickCount64();
        Send->ReservedLength = DataLength;
    }
    return Send;
}
    ...
    _In_ ULONG DataLength)
{
    ...
    if (Send->ProtocolConnection != NULL)
    {
        ZpConnection_RecordSendQueueDelay(Send->ProtocolConnection,
                                          Send->EnqueuedTickCount);
    }
    ...
    _In_ ULONG DataLength)
{
    ...
    if (TokenLength != 0)
    {
        PZP_TCP_SEND_IO Send = ZpTcpConnection_AllocateSend(TokenLength, NULL);
    ...
    _In_ ULONG PayloadLength)
{
    ...
    Status = ZpConnection_PrepareSend(ProtocolConnection,
                                      SendFlags,
                                      MessageType,
                                      Body,
                                      BodyLength,
    ...
    {
        Status = ZpConnection_ReserveSend(ProtocolConnection, RequiredSize);
        if (!NT_SUCCESS(Status))
        {
            RtlLeaveCriticalSection(&TcpConnection->Lock);
            ZpConnection_ReleaseSend(ProtocolConnection, &Message);
            return Status;
        }
    }
    Send = ZpStatus_IsSuccess(TransportStatus) ?
               ZpTcpConnection_AllocateSend(RequiredSize, ProtocolConnection) : NULL;
    if (ZpStatus_IsSuccess(TransportStatus) && Send == NULL)
    {
        ZpConnection_CompleteSend(ProtocolConnection, RequiredSize);
    ...
    return ZpConnection_NotifyMessageSent(ProtocolConnection,
                                          MessageType,
                                          sizeof(ULONG) + sizeof(BYTE) + Message.BodyLength +
                                              Message.PayloadLength);
}
    ...
    _In_ ULONG Error)
{
    ...
    else
    {
        ...
        if (Send->ProtocolConnection != NULL)
        {
            ZpConnection_CompleteSend(Send->ProtocolConnection, Send->ReservedLength);
        }
        Mem_Free(Send);
    }
    if (!ZpStatus_IsSuccess(Status))
