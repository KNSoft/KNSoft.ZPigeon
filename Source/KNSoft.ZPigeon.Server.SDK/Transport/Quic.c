    ...
    _In_ ZP_STATUS Status)
{
    ...
    if (QuicConnection->Connection != NULL)
    {
        if (ZpStatus_IsSuccess(QuicConnection->ShutdownStatus))
        {
            QuicConnection->ShutdownStatus = Status;
        }
    ...
    _In_ ULONG PayloadLength)
{
    ...
    {
        ZpServerQuic_SetShutdownStatus(QuicConnection, FailureStatus);
        MsQuicConnectionShutdown(QuicConnection->Connection,
                                 QUIC_CONNECTION_SHUTDOWN_FLAG_NONE,
                                 0);
    }
    ...
    _Inout_ QUIC_CONNECTION_EVENT* Event)
{
    ...
    switch (Event->Type)
    {
        ...
        case QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE:
            ...
            RtlAcquireSRWLockExclusive(&QuicConnection->Public.Lock);
            QuicConnection->Connection = NULL;
            RtlReleaseSRWLockExclusive(&QuicConnection->Public.Lock);
    ...
    switch (Event->Type)
    {
        case QUIC_LISTENER_EVENT_NEW_CONNECTION:
            ...
            Status = ZpServerConnection_Initialize(
                         ...
                         (const SOCKADDR*)Event->NEW_CONNECTION.Info->RemoteAddress,
                         QuicAddrGetFamily(Event->NEW_CONNECTION.Info->RemoteAddress) ==
