    ...
    _Out_ ZP_REQUEST_HANDLE* Request)
{
    ...
    if (ModuleId == 0 || OperationId == 0 || Callback == NULL || Request == NULL ||
        PayloadLength > ZP_REQUEST_MAX_PAYLOAD_SIZE ||
    ...
    if (NT_SUCCESS(Status))
    {
        Status = ConnectionObject->Send(ConnectionObject,
                                        ModuleId == ZP_SERVICE_MODULE_ID &&
                                                OperationId == ZP_SERVICE_OPERATION_CONFIGURE_ACCOUNT ?
                                            ZP_SEND_FLAG_SENSITIVE : 0,
                                        ZpMessageRequest,
                                        Header,
                                        sizeof(Header),
                                        Payload,
    ...
    _Out_ PZP_SERVER_CONNECTION_STATISTICS Statistics)
{
    ...
    Statistics->Transport = ConnectionObject->Transport;
    ProtocolConnection = ConnectionObject->ProtocolConnection;
    if (ProtocolConnection != NULL) ZpConnection_QueryStatistics(ProtocolConnection, &Network);
    RtlReleaseSRWLockShared(&ConnectionObject->Lock);
    if (ProtocolConnection == NULL) return STATUS_INVALID_DEVICE_STATE;
    Statistics->SentBytes = Network.SentBytes;
    Statistics->ReceivedBytes = Network.ReceivedBytes;
    Statistics->SentBitsPerSecond = Network.SentBitsPerSecond;
    Statistics->ReceivedBitsPerSecond = Network.ReceivedBitsPerSecond;
    Statistics->SentSampleTickCount = Network.SentSampleTickCount;
    Statistics->ReceivedSampleTickCount = Network.ReceivedSampleTickCount;
    Statistics->OutstandingSendBytes = Network.OutstandingSendBytes;
    Statistics->MaximumOutstandingSendBytes = Network.MaximumOutstandingSendBytes;
    Statistics->MaximumSendQueueDelayMilliseconds = Network.MaximumSendQueueDelayMilliseconds;
    Statistics->RejectedSends = Network.RejectedSends;
    Statistics->Policy = Network.Policy;
    return STATUS_SUCCESS;
}

NTSTATUS
NTAPI
ZpServer_QueryConnectionRemoteAddress(
    _In_ ZP_CONNECTION_HANDLE Connection,
    _Out_ PZP_IP_ADDRESS Address)
{
    PZP_CONNECTION_OBJECT ConnectionObject = Connection;

    if (ConnectionObject == NULL || Address == NULL) return STATUS_INVALID_PARAMETER;
    *Address = ConnectionObject->RemoteAddress;
    return STATUS_SUCCESS;
}

NTSTATUS
NTAPI
ZpServer_SetConnectionPolicy(
    _In_ ZP_CONNECTION_HANDLE Connection,
    _In_ PCZP_CONNECTION_POLICY Policy)
{
    PZP_CONNECTION_OBJECT ConnectionObject = Connection;
    PZP_CONNECTION ProtocolConnection;
    BYTE Body[ZP_CONNECTION_POLICY_WIRE_SIZE];
    ULONG BodyLength;
    NTSTATUS Status;

    if (ConnectionObject == NULL || Policy == NULL) return STATUS_INVALID_PARAMETER;
    Status = ZpMessage_EncodeConnectionPolicy(Policy, Body, sizeof(Body), &BodyLength);
    if (!NT_SUCCESS(Status)) return Status;
    RtlAcquireSRWLockShared(&ConnectionObject->Lock);
    ProtocolConnection = ConnectionObject->Phase == ZpConnectionPhaseReady ?
                             ConnectionObject->ProtocolConnection : NULL;
    Status = ProtocolConnection != NULL ?
                 ZpConnection_SetPolicy(ProtocolConnection, Policy) : STATUS_INVALID_DEVICE_STATE;
    RtlReleaseSRWLockShared(&ConnectionObject->Lock);
    return NT_SUCCESS(Status) ?
               ConnectionObject->Send(ConnectionObject,
                                      0,
                                      ZpMessageConnectionPolicy,
                                      Body,
                                      BodyLength,
                                      NULL,
                                      0) : Status;
