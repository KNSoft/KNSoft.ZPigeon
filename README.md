...
当前页面可探测远端主机上的 `cmd`、Windows PowerShell 和 PowerShell，新建/关闭多个 Shell、切换标签并进行完整 ConPTY 命令交互；也提供文件、进程、服务和注册表可视化管理。文件页支持分页浏览、属性、SHA-256、流式上传/下载、重命名和删除；进程页支持实时 CPU/内存等信息、映像路径和命令行详情及结束进程，并在页面不活跃时停止刷新；服务页支持属性、启动和停止。EventLog 支持 Bookmark 分页查询、频道启停和清除，不包含实时订阅。
